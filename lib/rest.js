...
OAE.tenantRouter.on('get', '/api/meeting/:meetingId/join', function(req, res) {
    MeetingsAPI.Meetings.getFullMeetingProfile(req.ctx, req.params.meetingId, function(err, meetingProfile) {
        ...
        // Clone groupProfile for passing it to Join
        var profile = (JSON.parse(JSON.stringify(groupProfile)));

        MeetingsAPI.Bbb.joinURL(req, profile, null, function(err, joinInfo) {
            if(err) {
                res.send(503, 'Fatal error');
            }
...
OAE.tenantRouter.on('get', '/api/meetup/:groupId/join', function(req, res) {
    PrincipalsAPI.getFullGroupProfile(req.ctx, req.params.groupId, function(err, groupProfile) {
        ...
        MeetingsAPI.Meetup.joinURL(req, profile, null, function(err, joinInfo) {
            if(err) {
                res.send(503, 'Fatal error');
            }
 ...
 * @Server      tenant
 * @Method      GET
 * @Path        /meetup/{groupId}/end
 * @PathParam   {string}                groupId             The id of the meetup to get
 * @HttpResponse                        200                 The meetup was successfully ended
 * @HttpResponse                        400                 groupId must be a valid resource id
 * @HttpResponse                        401                 You are not authorized to view this meetup
 * @HttpResponse                        404                 Could not find the specified meetup
 */
OAE.tenantRouter.on('post', '/api/meetup/:groupId/end', function(req, res) {
    PrincipalsAPI.getFullGroupProfile(req.ctx, req.params.groupId, function(err, groupProfile) {
        if (err) {
            return res.send(err.code, err.msg);
        }

        MeetingsAPI.Bbb.getEndMeetingURL(req, groupProfile, function(err, end) {
            if (err) {
                return res.send(503, 'Fatal error');
            }
            ...
            if ( end.returncode == 'success' ) {
                BBBProxy.executeBBBCall(end.url, function(err, endInfo) {
                    ...
                    MeetingsAPI.Bbb.getRecordingsURL(req, groupProfile, function(err, getRecordings) {
                        var interval_time = 1000;
                        var retries = 0;
                        ...
                        var polling_func = function(){
                            ...
                            BBBProxy.executeBBBCall(getRecordings.url, function(err, recordingsInfo) {
                                console.info(recordingsInfo);
                                if(recordingsInfo.returncode === 'SUCCESS' && recordingsInfo.recordings) {
                                    ...
                                    current = new Date();
                                    date = new Date(parseInt(recordings[highest].endTime));

                                    ContentAPI.createLink(req.ctx, date.toString(), 'description', 'private', recordings[highest].playback.format[0].url, members, [], function(err, contentObj) {
                                        if (err) {
                    ...
                    res.writeHead(200, endInfo);
                });
            } else {
                res.send(200, end.response);
            }

            MeetingsAPI.emit(MeetupsConstants.events.END_MEETUP, req.ctx, groupProfile, function(errs) {
